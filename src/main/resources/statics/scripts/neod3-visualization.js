...
function look_anka(value){
    ...
    try {
        ...
        console.log(query);
        var label = value["label"].toUpperCase();
        if(document.getElementById("tableData")){
            document.getElementById("tableData").setAttribute
...
function Neod3Renderer1() {
    ...
    function render(id, $container, visualization) {
        ...
        var links_copy = [];
        var nodes_copy = [];
        nodes_copy.push(nodes[0]);
        for(var i = 0;i < nodes.length;i++) {
            if(nodes[i].name.toString().indexOf("代码") != -1){
                nodes_id.push(nodes[i].id);
                nodes.splice(i,1);
            }
            else if(nodes[i].name.toString().indexOf("编码") != -1){
                nodes_id.push(nodes[i].id);
                nodes.splice(i,1);
            }
            else if(nodes[i].name.toString().indexOf("编号") != -1){
                nodes_id.push(nodes[i].id);
                nodes.splice(i,1);
            }
        }
        ...
        for(var i = 0;i < nodes.length;i++) {
            if(nodes_list.includes(nodes[i].name)){
                nodes_id.push(nodes[i].id);
                nodes.splice(i,1);
            }
            else {
                nodes_list.push(nodes[i].name);
                links_end.push(nodes[i].id);
                nodes_copy.push(nodes[i]);
            }
        }

        /*for(var i = 1;i < links.length;i++) {
            if(!links_end.includes(links[i].end)){
                links.splice(i,1);
            }
        }*/
        for(var i = 0;i < links.length;i++) {
            if (links_end.includes(links[i].end)) {
                links_copy.push(links[i]);
            }
        }
       links = links_copy;
       nodes = nodes_copy;
