...
public class ExtendActModelServiceImpl implements ExtendActModelerService {
    ...
    public String save(ExtendActModelEntity extendActModel) throws Exception {
        ...
        extendActModel.setId(CommUtils.uuid());
...
//		extendActModelEntity.setBapidList(UserUtils.getDateAuth(Constant.DataAuth.BAP_DATA.getValue()));
    ...
    @Override
    @Transactional
    public void deployFile(String modelId, MultipartFile bpmnfile) {
        if (StringUtils.isEmpty(modelId)) {
            throw new MyException("流程模型id不能为空!");
        }

        try {
            deployLocalFile(modelId, bpmnfile);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private void oldDeploy(String modelId) throws IOException {
...
//        String key=StringUtils.toStringByObject(extModel.getActKey())+"_"+modelId;
    ...
    private void deployLocalFile(String modelId, MultipartFile bpmnfile) throws Exception {
        Model model = repositoryService.getModel(modelId);

        //读取资源作为一个输入流
        FileInputStream fileInputStream = (FileInputStream)bpmnfile.getInputStream();
...
//        String key=StringUtils.toStringByObject(extModel.getActKey())+"_"+modelId;
        ...
        Deployment deployment = repositoryService.createDeployment()
				...
				.addString(modelId + ".bpmn20.xml", new String(bpmnfile.getBytes(), "utf-8"))
                .deploy();
